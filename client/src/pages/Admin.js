...
import { ModalAddProductItem } from "../components/modals/AddProductItem";
  ...
  return (
    <div className="container-admin flex flex-col h-screen min-w-full pt-14 ">
      ...
      <div className="flex flex-row p-2 space-x-4 justify-center ">
        ...
        <button className="p-1 border-2 rounded bg-slate-200 " onClick={() => {setAddProductItemVisible("true")}}>Add ProductItem</button>
      </div>
     {addProductCategoryVisible && <Modal show={addProductCategoryVisible} close={setAddProductCategoryVisible}/>}
     {addProductItemVisible && <ModalAddProductItem show={addProductItemVisible} close={setAddProductItemVisible}/>}
    </div>
  );
};
